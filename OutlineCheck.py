    ...
    def create_tool_icon(name):
        ...
        if name == "eyedropper":
            ...
            for pts, color in [
                ...
                ([QPoint(45, 20), QPoint(45, 5), QPoint(55, 10)], QColor(100, 100, 230)),
            ]:
...
class ImageAnalyzer:
    ...
    def analyze(self, pil_image):
        ...
        for y in range(1, h - 1):
            for x in range(1, w - 1):
                ...
                is_cl = any(self._check_pattern_match_optimized(rot, sub_mask) for rot in self.cl_rotations)

                if is_cl:
                    if color not in color_count: color_count[color] = []
                    color_count[color].append((x, y, "CL"))
                else:
                    is_pattern = any(self._check_pattern_match_optimized(rot, sub_mask) for rot in self.base_rotations)
                    if is_pattern:
                        if color not in color_count: color_count[color] = []
    ...
    def _categorize_errors(self, layers, img_np, bad_pixel_mask):
        ...
        for layer in other_layers:
            ...
            if any(self._check_full_pattern_match(rot, color_mask, bad_mask) for rot in self.osc_rotations):
                self._apply_category_style(layer, "OSC")
                if any(self._check_full_pattern_match(rot, color_mask, bad_mask) for rot in self.sc_rotations):
                    self._apply_category_style(layer, "SC")
            elif any(self._check_full_pattern_match(rot, color_mask, bad_mask) for rot in self.ot_patterns):
...
class OutlineCheckApp(QMainWindow):
    ...
    def _create_tool_panel(self, parent_layout):
        ...
        row.addWidget(self.btn_color_preview)
        ...
        btn_trans.setFixedSize(48, 48)
        btn_trans.setObjectName("toolBtn")
        btn_trans.setIcon(IconFactory.create_tool_icon("cross"))
        btn_trans.setIconSize(QSize(28, 28))
        btn_trans.clicked.connect(self.reset_to_transparent)
        row.addWidget(btn_trans)
    ...
    def _create_work_area(self, parent_layout):
        ...
        self.canvas_left.brushPainted.connect(self.paint_pixel)
        self.canvas_left.brushFinished.connect(self.on_brush_finished)
    ...
    def _create_layers_panel(self):
        ...
        self.layers_list.setHeaderLabels(["", LANGUAGES[self.current_lang]["layer_title"]]) 
    ...
    def update_layers_ui(self):
        ...
        if self.layers_list.topLevelItemCount() > 0 and not self.layers_list.selectedItems():
    ...
    def add_layer(self, name=None, image=None):
        ...
        layer_data = {"name": layer_name, "image": new_img, "visible": True, "locked": False}
    ...
    def move_layer(self, delta):
        ...
        if 0 <= new_idx < len(self.pixel_layers):
            self.pixel_layers[idx], self.pixel_layers[new_idx] = (self.pixel_layers[new_idx], self.pixel_layers[idx])
            self.update_layers_ui()
    ...
    def merge_layers(self):
        ...
        indices = sorted([self.layers_list.indexOfTopLevelItem(i) for i in selected], reverse=True)
        ...
        for i in range(1, len(indices)):
            new_img = Image.alpha_composite(new_img, self.pixel_layers[indices[i]]["image"])
    ...
    def handle_layer_click(self, item, column):
        ...
        if column == 0:
            ...
            item.setIcon(0, IconFactory.create_state_icon("eye", self.pixel_layers[idx]["visible"]))
            self.compose_final_image()
    ...
    def rebuild_error_tree(self):
        ...
        for color in self.analysis_result.unique_colors:
            ...
            color_layers = [l for l in self.analysis_result.layers if l.color_key == color_tuple]
    ...
    def on_selection_changed(self):
        ...
        for i in range(root.childCount()):
            parent = root.child(i)
            has_selected = any(parent.child(j).isSelected() for j in range(parent.childCount()))
            if has_selected:
    ...
    def update_canvas_views(self):
        ...
        self.canvas_left.set_image(pix, self.analysis_result.pixel_to_layers, reset_view=False)
    ...
    def update_picker_button_ui(self):
        ...
        if self.replacement_color is None:
            ...
            painter.drawLine(offset + 3, offset + 5, offset + rect_size - 7, offset + rect_size - 5)
        else:
    ...
    def toggle_brush_mode(self):
        new_mode = not self.canvas_left.brush_mode
        self._set_tool_mode(picker=False, brush=new_mode)

    def _set_tool_mode(self, picker, brush):
        self.canvas_left.picker_mode = picker
        self.canvas_left.brush_mode = brush
        
        self.btn_eye_dropper.setStyleSheet("background-color: #00c3ff; border: 1px solid white;" if picker else "")
        self.btn_brush.setStyleSheet("background-color: #00c3ff; border: 1px solid white;" if brush else "")
    ...
    def paint_pixel(self, x, y):
        ...
        if not items or not self.pixel_layers: return
    ...
    def fix_selected_layers(self):
        ...
        ids = [i.data(0, Qt.ItemDataRole.UserRole) for i in selected if i.data(0, Qt.ItemDataRole.UserRole) is not None]
        if not ids: return
