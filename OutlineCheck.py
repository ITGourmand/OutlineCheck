...
class PixelCanvas(QWidget):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.RightButton or self.space_pressed:
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
            ...
            if self.brush_mode and event.buttons() & Qt.MouseButton.LeftButton:
                x = int(event.position().x() / self.zoom + self.offset_x)
                y = int(event.position().y() / self.zoom + self.offset_y)
                self.brushPainted.emit(x, y)
...
class OutlineCheckApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn_brush.setIcon(self.create_icon_svg("brush"))
    ...
    def create_icon_svg(self, name):
        ...
        if name == "eyedropper":
            ...
            painter.setPen(QPen(black, 14)); painter.drawLine(20, 55, 44, 20)
            painter.setPen(QPen(black, 20)); painter.setBrush(black)
            painter.drawEllipse(40, 10, 5, 20)
            painter.setPen(QPen(black, 2))
            painter.drawPolygon([QPoint(45, 35), QPoint(45, 5), QPoint(60, 10)])

            painter.setPen(QPen(QColor(200, 140, 10), 7)); painter.drawLine(20, 55, 44, 20)
            painter.setPen(QPen(white, 10)); painter.setBrush(white)
            painter.drawEllipse(40, 10, 5, 20)
            ...
            for pts, color in [
                ...
                painter.setBrush(color); painter.setPen(QPen(color, 2))
    ...
    def update_picker_button_ui(self):
        ...
        if self.replacement_color is None:
            ...
            painter.drawRoundedRect(offset_x, offset_y, rect_size, rect_size, 8, 8)
            painter.setPen(QPen(Qt.GlobalColor.red, 3))
            painter.drawLine(offset_x + 5, offset_y + 5, offset_x + rect_size - 5, offset_y + rect_size - 5)
        else:
    ...
    def toggle_picker_mode(self):
        self.canvas_left.brush_mode = False
    ...
    def toggle_brush_mode(self):
        ...
        self.canvas_left.brush_mode = not self.canvas_left.brush_mode
        
        if self.canvas_left.brush_mode:
            self.setCursor(Qt.CursorShape.PointingHandCursor)
            self.btn_brush.setStyleSheet("background-color: #00c3ff; border: 1px solid white;")
        else:
            self.setCursor(Qt.CursorShape.ArrowCursor)
            self.btn_brush.setStyleSheet("")
    ...
    def paint_pixel(self, x, y):
        ...
        current_pixel = self.current_img.getpixel((x, y))
